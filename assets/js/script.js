...
(function() {
    ...
    function setupTimerControls() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target instanceof Node) {
                if (timerSetDropdown && timerControlDropdown && !timerSetDropdown.contains(e.target) && !timerControlDropdown.contains(e.target) && !timerIcon.contains(e.target)) {
                    timerSetDropdown.classList.add('hidden');
                    timerControlDropdown.classList.add('hidden');
                }
            }
    ...
    function handleStartNow() {
        // Points to multi-learn-hub.html as per recent architecture changes
        window.location.href = 'multi-learn-hub.html';
    }

    function handleSampleAnswer(index) {
        ...
        options.forEach(opt => {
            ...
            // Fix: Escape dot in selector for Tailwind class w-2.5
            const dot = opt.querySelector('.w-2\\.5');
        ...
        const dot = selected.querySelector('.w-2\\.5');
        if (dot) dot.classList.add('opacity-100');
        
        if (index === 1) { // Correct answer: Decreases
            selected.classList.add('bg-green-100', 'border-green-500', 'dark:bg-green-900/30');
            feedback.innerHTML = '<span class="text-green-600 font-bold">Correct!</span> Light slows down in a denser medium, reducing its wavelength.';
        } else {
            selected.classList.add('bg-red-100', 'border-red-500', 'dark:bg-red-900/30');
            feedback.innerHTML = '<span class="text-red-600 font-bold">Incorrect.</span> Try again! Hint: v = fλ.';
        }
    ...
    Object.assign(window, {
        ...
        MapsToVideos: navigateToVideos,
        MapsToMultiLearn: navigateToMultiLearn,
        MapsToCoffee: navigateToCoffee
    });
})();
